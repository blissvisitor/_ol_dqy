 ...
 */
define(['ol','css!olmap/modules/measure/measuretool.css'], function(ol,css) {
    ol.control.MeasureTool = function(opt_options) {
        ...
        html += '<li><input type="button" value="长度"></li>';
        html += '<li><input type="button" value="面积"></li>';
        html += '<li><input type="checkbox" style="display:none;" value="no"></li>';
        html += '</ul>';
    ...
    ol.control.MeasureTool.prototype.mapmeasure = function(typeSelect) {
        ...
        var map = this.getMap();
        if ($.inArray(vector, map.getLayers().getArray()) == -1) {
            map.addLayer(vector);
        }
        ...
        function addInteraction() {
            var type = (typeSelect.value == '面积' ? 'Polygon' : 'LineString');
